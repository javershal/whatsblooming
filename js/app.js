(async function () {
  ...
  document.getElementById('current-month').textContent =
    `Showing plants blooming in ${MONTH_NAMES[currentMonth - 1]}`;
  ...
  const blooming = plants.filter(p => p.bloom_months.includes(currentMonth));
  ...
  for (const plant of blooming) {
    const photoHtml = plant.photo_url
      ? `<img
           ...
           loading="lazy"
           onerror="this.closest('.plant-photo-wrap').innerHTML='<div class=\\"photo-missing\\">No photo available</div>'"
         >`
